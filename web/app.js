...
function render(result){const runningRisk=result.running_risk_score ?? result.combined_risk_score;const segmentRisk=result.combined_risk_score;const risk=Math.round(runningRisk*100);$("risk-score").textContent=risk;$("gauge").style.color=risk>=70?"var(--danger)":"var(--teal)";$("risk-label").textContent=result.evidence_segments<3?`Collecting evidence (${result.evidence_segments}/3)`:risk>=70?"Elevated synthetic-voice risk":risk>=45?"Review recommended":"Low synthetic-voice risk";$("score-context").textContent=`Latest segment: ${Math.round(segmentRisk*100)}% · ${result.alert?"Alert active":"Alert threshold: 70%"}`;$("result-title").textContent=`Call risk · ${risk}%`;$("tier1-score").textContent=`${Math.round(result.tier1.score*100)}%`;$("tier1-latency").textContent=`${result.tier1.latency_ms} ms`;$("tier1-label").textContent=`Trained checkpoint · ${result.tier1.label}`;$("tier2-score").textContent=`${Math.round(result.tier2.score*100)}%`;$("tier2-confidence").textContent=`${Math.round(result.tier2.confidence*100)}% confidence`;$("tier2-label").textContent=`Heuristic ensemble · ${result.tier2.label}`;const c=result.consistency_check;$("consistency").textContent=c.flag.replaceAll("_"," ");$("consistency-copy").textContent=c.similarity_score==null?"No reference available":`${Math.round(c.similarity_score*100)}% similarity to known voice`;$("alert-box").classList.toggle("hidden",!result.alert);$("api-state").textContent="LIVE";renderChart();renderFeatures(result.feature_breakdown);renderBars(result.tier2.encoder_contributions);}
function renderChart(){const chart=$("chart");chart.innerHTML="";state.results.slice(-12).forEach(r=>{const risk=r.combined_risk_score;const b=document.createElement("div");b.className=`chart-bar ${risk>=.7?"high":""}`;b.style.height=`${Math.max(8,risk*88)}px`;b.title=`${Math.round(risk*100)}% segment risk`;chart.append(b)});$("segment-count").textContent=`${state.results.length} analyzed`;}
